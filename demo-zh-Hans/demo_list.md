...
证书安全问题：每份 Thor 证书都是在第一次启动时随机生成（Thor 用户间彼此不同），不上传服务器，只存放在设备本地 keychain，Thor 只是把`证书手动随机生成`自动化了。


另外，所以用 Thor 抓到的数据都只会存储在手机本地，并没有任何上传数据（Thor 只是一个本地单机应用，甚至没有服务器）。
